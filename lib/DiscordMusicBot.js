    ...
    console.log("Manager instantiated:", !!this.manager);
      ...
      .on(
        ...
        /** @param {EpicPlayer} player */ async (player, track) => {
          ...
          // Actualizar el estado del canal con el nombre de la canción y el emoji del perfil
          try {
            const emoji = client.config.voiceStatusEmoji || "";
            const statusText = `${emoji} ${title}`.trim().substring(0, 192);
            await client.rest.put(
              `/channels/${player.options.voiceChannelId}/voice-status`,
              { body: { status: statusText } }
            );
          } catch (err) {
            client.error(`No se pudo actualizar el estado de voz: ${err.message}`);
          }
       }
      )
