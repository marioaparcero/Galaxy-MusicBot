...
module.exports = async (client, interaction) => {
	...
	if (
		...
		return await interaction.reply({ embeds: [sameEmbed], flags: 64 });
	}
	...
	if (property === "Replay") {
		const previousSongs = await player.queue.getPrevious();
		const previousSong = previousSongs && previousSongs.length > 0 ? previousSongs[0] : null;
		const currentSong = await player.queue.getCurrent();
        if (!previousSong) {
		...
		if (previousSong !== currentSong) {
			await player.queue.addPrevious(currentSong);
	...
	if (property === "PlayAndPause") {
		if (!player || (!player.playing && (player.queue.length + (player.queue.current ? 1 : 0)) === 0)) {
			const msg = await interaction.channel.send({
                               flags: 64,
				embeds: [
					new EmbedBuilder()
			...
			client.warn(`Reproductor: ${ player.options.guildId } | El reproductor ha sido ${ player.paused? "pausado" : "reanudado" }`);
	...
	if (property === "Loop") {
		...
		client.warn(`Reproductor: ${player.options.guildId} | Bucle ${player.trackRepeat ? "activado" : player.queueRepeat ? "en cola" : "apagado"} en el reproductor`);
		...
		if (queue.length === 0) {
			...
			const queueEmbed = new EmbedBuilder()
				...
				.addFields(
					...
					// {
					// 	name: `**${(player.queue.length + (player.queue.current ? 1 : 0)) - 1} canciones en espera**`,
					// 	value: `\nNo hay canciones en espera. ¡Agrega una!`
					// }
				);
